...
describe('Todos', () => {
  ...
  describe('#handleDelete', () => {
    ...
    it('should remove local data when handleDelete called', async () => {
      ...
      await act(async () => {
        const todoListComponent = wrapper.find('TodoList');
        await todoListComponent.props().handleDelete('6051c79d06526d5760007b7f');
        wrapper.update();
      });
      const updatedTodoListComponent = wrapper.find('TodoList');

      expect(updatedTodoListComponent.props().todos).toEqual(expectedDeletedData);
    });
  });
  ...
  describe('#handleUpdate', () => {
    ...
    it('should update local data when handleUpdate called', async () => {
      const expectedUpdatedData = [
        { _id: '6051c7bd06526d5760007b81', title: 'Hello World', description: 'Duaaar ini todo list pertama hore' },
        { _id: '6051c7ae06526d5760007b80', title: 'banana potato', description: 'lorem ipsum dolor sit amet' },
        { _id: '6051c79d06526d5760007b7f', title: 'updated title', description: 'updated description' }
      ];
      fetchAll.mockResolvedValue(mockFetchedData);
      let wrapper;
      await act(async () => {
      ...
      await act(async () => {
        const todoFormComponent = wrapper.find('TodoForm');
        await todoFormComponent.props().handleUpdate('6051c79d06526d5760007b7f', 'updated title', 'updated description');
        wrapper.update();
      });
      const updatedTodoListComponent = wrapper.find('TodoList');

      expect(updatedTodoListComponent.props().todos).toEqual(expectedUpdatedData);
    });
  });
